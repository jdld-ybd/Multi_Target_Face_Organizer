...
# 步骤3: 处理每张图片，匹配并移动
def process_images(files, target_encodings):
    # 创建输出文件夹结构
    ...
    for file in files:
        ...
        try:
            ...
            if not face_encodings:
                ...
                print(f"{file}：无人脸，已移动到 else 文件夹")
                continue
            ...
            if min_distance < TOLERANCE:
                ...
                shutil.move(input_path, output_path)
                print(f"{file}：匹配 {best_match} (距离: {min_distance:.2f})，已移动到 {folder_name}")
            else:
                # 不匹配，移到else
                output_path = os.path.join(OUTPUT_FOLDER, "else",file)
                shutil.move(input_path, output_path)
                print(f"{file}：不匹配任何目标 (最小距离: {min_distance:.2f})，已移动到 else 文件夹")
...
if __name__ == "__main__":
    # 指定目标人脸文件列表（可手动修改）
    target_files = ["y.jpg", "t.jpg", "ya.jpg", "d.jpg"]  # 示例，添加更多
    ...
    process_images(files, target_encodings)
    print("处理完成！")
